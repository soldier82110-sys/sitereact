...
import { ApiKeyIcon, CheckCircleIcon, XCircleIcon, EditIcon, TrashIcon, LogoIcon, UserIcon, EmailIcon, CpuIcon, PlusIcon, DatabaseIcon, ShareIcon, FontIcon } from '../../../components/icons';
import { useAppSettings, useToast } from '../../../contexts';
import { Marja, UploadedFontFamily } from '../../../types';
...
type ActiveTab = 'general' | 'ai_processing' | 'appearance' | 'seo' | 'maraji' | 'admins' | 'login' | 'shareCard' | 'font';
...
const SiteSettings: React.FC = () => {
    ...
    const [itemToDelete, setItemToDelete] = useState<{ id: number; name: string; type: 'prompt' | 'marja' | 'admin' | 'selfHostedModel' | 'fontFamily' } | null>(null);
    ...
    const [editingFontFamily, setEditingFontFamily] = useState<UploadedFontFamily | null>(null);
    ...
    const handleSaveSection = (section: keyof typeof settings | 'ai_processing' | 'appearance' ) => {
        if (section === 'ai_processing') {
            setIsAiConfirmOpen(true);
            return;
        }

        let updatePayload: Partial<typeof settings> = {};
        if (section === 'appearance') {
            ...
            // @ts-ignore
            updatePayload = { [section]: localSettings[section] };
        }
    ...
    const handleDeleteClick = (id: number, name: string, type: 'prompt' | 'marja' | 'admin' | 'selfHostedModel' | 'fontFamily') => {
    ...
    const handleSaveSelfHostedModel = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingSelfHostedModel) {
            ...
            const newModel: SelfHostedModel = { id: Date.now(), name, apiUrl, apiKey, status: 'idle' };
    ...
    const handleSaveUploadedFontFamily = (fontFamily: UploadedFontFamily) => {
        setLocalSettings(prev => {
            const familyExists = prev.fontSettings.uploadedFamilies.some(f => f.id === fontFamily.id);
            const newFamilies = familyExists
                ? prev.fontSettings.uploadedFamilies.map(f => f.id === fontFamily.id ? fontFamily : f)
                : [...prev.fontSettings.uploadedFamilies, fontFamily];

            if (familyExists) {
                showToast('خانواده فونت ویرایش شد.');
            } else {
                showToast('خانواده فونت جدید اضافه شد.');
            }
    ...
    const handleOpenFontFamilyModal = (family: UploadedFontFamily | null = null) => {
    ...
    const parsedUrl = useMemo(() => {
        ...
        try {
            ...
            if (match) return { host: match[1] || '', port: match[2] || '', path: match[3] || '' };
        ...
        switch (itemToDelete.type) {
            ...
            case 'fontFamily':
                setLocalSettings(prev => ({
                    ...
                    fontSettings: {
                        ...prev.fontSettings,
                        uploadedFamilies: prev.fontSettings.uploadedFamilies.filter(f => f.id !== itemToDelete.id)
                    }
                }));
                showToast('خانواده فونت حذف شد.');
                break;
        }
    ...
    const getDeleteModalTitle = () => {
        ...
        switch (itemToDelete.type) {
            ...
            case 'fontFamily': return 'حذف خانواده فونت';
            default: return 'حذف';
        }
    };
        ...
        <div>
            ...
            <div className="border-b border-gray-200 dark:border-navy-gray-light mb-6">
                <nav className="flex flex-wrap gap-x-4 gap-y-2">
                    <TabButton tabName="general" label="عمومی" />
                    <TabButton tabName="ai_processing" label="هوش مصنوعی" />
                    <TabButton tabName="appearance" label="ظاهری" />
                    <TabButton tabName="font" label="مدیریت فونت" />
                    <TabButton tabName="seo" label="سئو" />
                    <TabButton tabName="login" label="صفحه ورود" />
                    <TabButton tabName="shareCard" label="کارت اشتراک‌گذاری" />
                    <TabButton tabName="maraji" label="مراجع تقلید" />
                    <TabButton tabName="admins" label="مدیران" />
                </nav>
            </div>
            ...
            <div className="space-y-8">
                    ...
                    <div className="space-y-6">
                        <Card>
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200">فونت‌های آپلود شده</h3>
                                <Button onClick={() => handleOpenFontFamilyModal(null)}>+ آپلود خانواده فونت</Button>
                            </div>
                            <div className="space-y-2">
                                {localSettings.fontSettings.uploadedFamilies.map(family => (
                                    <div key={family.id} className="grid grid-cols-1 md:grid-cols-3 items-center gap-4 p-2 rounded hover:bg-gray-100 dark:hover:bg-navy-gray-light">
                                        <div className="font-semibold">{family.name}</div>
                                        <div className="text-gray-500 dark:text-gray-400 text-sm">{family.files.length} فایل (وزن)</div>
                                        <div className="flex justify-start md:justify-end items-center gap-2">
                                            <button onClick={() => handleOpenFontFamilyModal(family)} aria-label="ویرایش" className="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-navy-gray"><EditIcon className="w-5 h-5"/></button>
                                            <button onClick={() => handleDeleteClick(family.id, family.name, 'fontFamily')} aria-label="حذف" className="p-1.5 rounded hover:bg-brick/20"><TrashIcon className="w-5 h-5 text-brick"/></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <Card>
                             <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">اعمال فونت‌ها</h3>
                             <div className="space-y-4 max-w-sm">
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">متن اصلی سایت (Body)</label>
                                    <select name="fontSettings.application.body" value={localSettings.fontSettings.application.body} onChange={handleSettingsChange} className="w-full bg-off-white/50 dark:bg-navy-gray-dark/50 border border-gray-300 dark:border-navy-gray-light rounded-lg p-2.5">
                                        <option value="default">پیش‌فرض (Vazirmatn)</option>
                                        {localSettings.fontSettings.uploadedFamilies.map(f => <option key={f.id} value={f.id.toString()}>{f.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">عناوین (Headings)</label>
                                    <select name="fontSettings.application.headings" value={localSettings.fontSettings.application.headings} onChange={handleSettingsChange} className="w-full bg-off-white/50 dark:bg-navy-gray-dark/50 border border-gray-300 dark:border-navy-gray-light rounded-lg p-2.5">
                                        <option value="default">پیش‌فرض (Vazirmatn)</option>
                                        {localSettings.fontSettings.uploadedFamilies.map(f => <option key={f.id} value={f.id.toString()}>{f.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">ورودی متن چت</label>
                                    <select name="fontSettings.application.chatInput" value={localSettings.fontSettings.application.chatInput} onChange={handleSettingsChange} className="w-full bg-off-white/50 dark:bg-navy-gray-dark/50 border border-gray-300 dark:border-navy-gray-light rounded-lg p-2.5">
                                        <option value="default">پیش‌فرض (Vazirmatn)</option>
                                        {localSettings.fontSettings.uploadedFamilies.map(f => <option key={f.id} value={f.id.toString()}>{f.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">فونت کارت اشتراک‌گذاری</label>
                                    <select name="fontSettings.application.shareCard" value={localSettings.fontSettings.application.shareCard} onChange={handleSettingsChange} className="w-full bg-off-white/50 dark:bg-navy-gray-dark/50 border border-gray-300 dark:border-navy-gray-light rounded-lg p-2.5">
                                        <option value="default">پیش‌فرض (Vazirmatn)</option>
                                        {localSettings.fontSettings.uploadedFamilies.map(f => <option key={f.id} value={f.id.toString()}>{f.name}</option>)}
                                    </select>
                                </div>
                         ...
                         <div className="flex justify-end mt-6">
                            <Button onClick={() => handleSaveSection('fontSettings')}>ذخیره تنظیمات فونت</Button>
                        </div>
                    </div>
                )}
                    ...
                    <Card>
                        ...
                        <div className="space-y-6 max-w-lg">
                            ...
                            <div>
                                ...
                                <Input name="loginPage.title" value={localSettings.loginPage.title} onChange={handleSettingsChange} />
                            </div>
                             <div>
                                <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">زیرنویس</label>
                                <textarea name="loginPage.subtitle" rows={4} className="w-full bg-off-white/50 dark:bg-navy-gray-dark/50 border rounded-lg p-3" value={localSettings.loginPage.subtitle} onChange={handleSettingsChange} />
                ...
                {activeTab === 'shareCard' && (
                    <Card>
                        <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">قالب کارت اشتراک‌گذاری</h3>
                         <div className="bg-amber-50 dark:bg-amber-900/30 p-4 rounded-lg text-amber-800 dark:text-amber-200 mb-4">
                            <h4 className="font-bold">هشدار امنیتی</h4>
                            <p className="text-sm">ویرایش مستقیم HTML و CSS می‌تواند مخاطرات امنیتی (مانند XSS) به همراه داشته باشد. لطفاً فقط از کدهای معتبر استفاده کنید و از وارد کردن اسکریپت‌های ناشناس خودداری نمایید.</p>
                        </div>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">کد HTML/CSS قالب</label>
                                <textarea
                                    name="shareCardTemplate"
                                    rows={16}
                                    className="w-full bg-off-white/50 dark:bg-navy-gray-dark/50 border border-gray-300 dark:border-navy-gray-light rounded-lg p-3 font-mono text-sm"
                                    value={localSettings.shareCardTemplate}
                                    onChange={handleSettingsChange}
                                    style={{ direction: 'ltr', textAlign: 'left' }}
                                />
                                 <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                    از پلیس‌هولدرهای <code>{"{{QUESTION_TEXT}}"}</code> و <code>{"{{ANSWER_TEXT}}"}</code> برای نمایش محتوای پویا استفاده کنید.
                                </p>
                            </div>
                        </div>
                        <div className="flex justify-end mt-6">
                            <Button onClick={() => handleSaveSection('shareCardTemplate')}>ذخیره قالب کارت</Button>
                        </div>
                    </Card>
                )}
            ...
            <FontUploadModal
                isOpen={isFontFamilyModalOpen}
                onClose={() => setIsFontFamilyModalOpen(false)}
                onSave={handleSaveUploadedFontFamily}
                editingFamily={editingFontFamily}
            />
