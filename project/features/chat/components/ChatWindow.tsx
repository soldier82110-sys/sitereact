...
import { CopyIcon, ThumbsUpIcon, ThumbsDownIcon, FlagIcon, RetryIcon, XCircleIcon, ShareIcon } from '../../../components/icons';
...
interface ChatWindowProps {
    ...
    onShareClick: (userMessage: Message, aiMessage: Message) => void;
}
...
const FeedbackActions: React.FC<{ messageText: string, onReportClick: () => void; onShareClick: () => void; }> = ({ messageText, onReportClick, onShareClick }) => {
  ...
  return (
    <div className="flex items-center gap-1.5 ml-auto">
      ...
      <button onClick={handleCopy} aria-label="کپی کردن پیام" className="p-1.5 rounded-full text-gray-500 hover:bg-gray-200 dark:hover:bg-navy-gray-light transition-colors">
        <CopyIcon className="w-5 h-5" />
      </button>
      <button
        onClick={handleLike}
        aria-label="پسندیدن"
        className={`p-1.5 rounded-full transition-colors ${feedback === 'liked' ? 'text-turquoise bg-turquoise/10' : 'text-gray-500 hover:bg-gray-200 dark:hover:bg-navy-gray-light'}`}
      >
      ...
      <button
        ...
        aria-label="نپسندیدن"
        className={`p-1.5 rounded-full transition-colors ${feedback === 'disliked' ? 'text-brick bg-brick/10' : 'text-gray-500 hover:bg-gray-200 dark:hover:bg-navy-gray-light'}`}
      >
        <ThumbsDownIcon className="w-5 h-5" />
      </button>
      <button onClick={onReportClick} aria-label="گزارش پیام" className="p-1.5 rounded-full text-gray-500 hover:bg-gray-200 dark:hover:bg-navy-gray-light transition-colors">
...
const ChatWindow: React.FC<ChatWindowProps> = ({ messages, isAiTyping, onReportClick, onRetryMessage, onShareClick }) => {
    const { user } = useUser();
                                ...
                                <div className="flex flex-col items-start gap-2 max-w-lg">
                                    <div className="bg-gray-100 dark:bg-navy-gray p-4 rounded-b-xl rounded-tr-xl shadow-sm">
                                     ...
                                     <FeedbackActions
                                        ...
                                        onShareClick={() => {
                                            let userMessageForShare: Message | undefined;
                                            // Find the preceding user message to create a Q&A pair
                                            for (let i = index - 1; i >= 0; i--) {
                                                const prevItem = messagesWithSeparators[i];
                                                if (!('type' in prevItem) && prevItem.sender === 'user') {
                                                    userMessageForShare = prevItem;
                                                    break;
                                                }
                                            }
                                            if (userMessageForShare) {
                                                onShareClick(userMessageForShare, msg);
                                            }
                                        }}
                                    />
                                </div>
