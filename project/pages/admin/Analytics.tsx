...
// Fix: Import mock data from mockData.ts
import { initialReports, reportCategoryLabels } from '../../services/mockData';
import { ThumbsUpIcon, ThumbsDownIcon, FlagIcon } from '../../components/chat/icons';
...
const Analytics: React.FC = () => {
    // Fix: Explicitly type accumulator for reduce to ensure correct type inference
    const feedbackCounts = initialReports.reduce<Record<string, number>>((acc, report) => {
        const feedback = report.feedback || 'none';
        acc[feedback] = (acc[feedback] || 0) + 1;
        return acc;
    }, { liked: 0, disliked: 0, reported: 0 });

    const categoryCounts = initialReports.reduce<Record<string, number>>((acc, report) => {
        if (report.category) {
