...
import { useAppSettings, useUser, useToast } from '../contexts/ThemeContext';
...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Update user context
    const userName = activeTab === 'register' ? name : (email.split('@')[0] || 'کاربر');
    const userDetails = {
      name: userName,
      email: email
    };
    ...
    setTimeout(() => {
      ...
      showToast(`${userName} عزیز، خوش آمدید!`);
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-off-white to-turquoise/10 dark:from-navy-gray-dark dark:to-turquoise/10">
      <Card className="w-full max-w-md mx-4">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold mb-2" dangerouslySetInnerHTML={{ __html: settings.loginPage.title }}></h1>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <>
              ...
              <div>
                <Input 
                    ...
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className={emailError ? 'border-brick focus:ring-brick' : ''}
                />
          ...
          <div className="flex items-center my-4">
              ...
              <span className="mx-4 text-sm text-gray-500">یا</span>
              <hr className="flex-grow border-gray-300 dark:border-navy-gray-light" />
          </div>
