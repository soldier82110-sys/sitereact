    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-8">
                <h2 className="text-3xl font-bold">مشاهده کاربر: {user.name}</h2>
            ...
            <Card className="mb-8">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-sm">
                    ...
                    <div><span className="font-semibold">توکن باقی‌مانده:</span> {user.tokens.toLocaleString()}</div>
                 ...
                 <div className="flex flex-col md:flex-row gap-6 h-[60vh]">
                    ...
                    <Card className="w-full md:w-1/3 !p-0 overflow-y-auto">
                        ...
                        {user.conversations.length > 0 ? (
                            ...
                            <p className="p-4 text-center text-gray-500">این کاربر هنوز گفتگویی نداشته است.</p>
                        )}
                    </Card>
                            ...
                            {activeConversation ? activeConversation.messages.map((msg, index) => (
                                <div key={index} className={`flex w-full ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                 ...
                                 <div className="flex items-center justify-center h-full">
                                    <p className="text-gray-500">یک گفتگو را برای مشاهده انتخاب کنید.</p>
                                </div>
                            )}
                        </div>
                    </Card>
                        ...
                        <table className="w-full text-right">
                            <thead className="border-b border-gray-200 dark:border-navy-gray-light">
                                <tr>
                                    ...
                                    <th className="p-3">مقدار توکن</th>
                            ...
                            <tbody>
                                {mockTransactions.map(tx => (
                                    <tr key={tx.id} className="border-b border-gray-100 dark:border-navy-gray hover:bg-gray-50 dark:hover:bg-navy-gray-light/50">
                                        ...
                                        <td className="p-3 font-semibold text-green-600 dark:text-green-400">{tx.tokens}</td>
