    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold">تنظیمات هدیه معنوی (توکن صلواتی)</h3>
            </div>
            ...
            <div className={`space-y-4 max-w-md transition-opacity duration-300 ${localSettings.enabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                <div>
                    <label className="block text-sm font-medium mb-1">مدت انتظار (ثانیه)</label>
                ...
                <div>
                    <label className="block text-sm font-medium mb-1">تعداد توکن در هر بار</label>
                ...
                <div>
                    <label className="block text-sm font-medium mb-1">حداکثر توکن هدیه در روز</label>
...
const Financials: React.FC = () => {
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold mb-8">مدیریت مالی</h2>
            ...
            <Card>
                {activeTab === 'packages' && (
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-semibold">بسته‌های توکن</h3>
                            <Button onClick={() => handleOpenModal('packages')}>+ ایجاد بسته جدید</Button>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-right">
                                <thead className="border-b"><tr><th className="p-3">نام بسته</th><th className="p-3">تعداد توکن</th><th className="p-3">قیمت (تومان)</th><th className="p-3">عملیات</th></tr></thead>
                                <tbody>{mockPackages.map(p => (<tr key={p.id} className="border-b dark:border-navy-gray">
                ...
                {activeTab === 'discounts' && (
                     <div>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-semibold">کدهای تخفیف</h3>
                            <Button onClick={() => handleOpenModal('discounts')}>+ ایجاد کد تخفیف</Button>
                        </div>
                         <div className="overflow-x-auto">
                            <table className="w-full text-right">
                                <thead className="border-b"><tr><th className="p-3">کد</th><th className="p-3">مقدار</th><th className="p-3">تاریخ انقضا</th><th className="p-3">استفاده (شده/کل)</th><th className="p-3">عملیات</th></tr></thead>
                                <tbody>{mockDiscounts.map(d => (<tr key={d.id} className="border-b dark:border-navy-gray">
                ...
                {activeTab === 'gifts' && (
                     <div>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-semibold">کارت‌های هدیه</h3>
                            <Button onClick={() => handleOpenModal('gifts')}>+ ایجاد کارت هدیه</Button>
                        </div>
                         <div className="overflow-x-auto">
                            <table className="w-full text-right">
                                <thead className="border-b"><tr><th className="p-3">عنوان</th><th className="p-3">کد</th><th className="p-3">مقدار توکن</th><th className="p-3">استفاده شده توسط</th><th className="p-3">عملیات</th></tr></thead>
                                <tbody>{mockGifts.map(g => (<tr key={g.id} className="border-b dark:border-navy-gray">
            ...
            <Modal isOpen={isUsageModalOpen} onClose={() => setIsUsageModalOpen(false)} title="مشاهده استفاده از کد تخفیف">
                <div className="max-h-80 overflow-y-auto">
                    <table className="w-full text-right">
                        <thead className="border-b"><tr><th className="p-2">ایمیل کاربر</th><th className="p-2">تاریخ استفاده</th></tr></thead>
                        <tbody>{mockUsage.map((u, i) => <tr key={i}><td className="p-2">{u.email}</td><td className="p-2">{u.date}</td></tr>)}</tbody>
                    </table>
                </div>
